        ...
        <>
            <main className="min-h-screen pt-32 pb-20 bg-[#f4f6fb]">
                <div className="max-w-7xl mx-auto px-6 md:px-10">
                    ...
                    <Link
                        href="/#produtos"
                        className="mb-8 inline-flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-gray-200 text-sm font-bold text-[#1e3a8a] shadow-sm hover:bg-gray-50 transition-all cursor-pointer"
                    >
                        ...
                        <div className="lg:col-span-5">
                            <div className="bg-white rounded-3xl p-8 border border-gray-200 shadow-xl sticky top-32">
                                ...
                                <div className="flex items-center gap-2 mb-6">
                                    <span className={`w-3 h-3 rounded-full ${product.available ? 'animate-pulse' : ''}`} style={{ background: product.available ? '#22c55e' : '#ef4444' }} />
                                    <span className={`text-[13px] font-bold uppercase tracking-tight ${product.available ? 'text-green-600' : 'text-red-600'}`}>
                                        {product.available ? 'Disponível para hoje' : 'Indisponível no momento'}
                                    </span>
                                </div>

                                <div className={`py-4 px-6 rounded-xl font-bold text-sm mb-8 flex items-center justify-center border ${!product.available ? 'bg-slate-50 text-[#1e3a8a] border-slate-200' : 'bg-[#1e3a8a] text-white border-transparent shadow-lg'}`}>
                                ...
                                {product.tags && (
                                    <div className="flex flex-wrap gap-2 mb-10">
                                        {product.tags.split(',').map((tag) => (
                                            <span key={tag} className="px-3 py-1 bg-blue-50 text-[#1e3a8a] text-xs font-bold rounded-lg border border-blue-100 uppercase tracking-tighter">
                                                {tag.trim()}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {/* Descrição */}
                                <div className="mb-10">
                                    <h2 className="text-lg font-bold text-gray-900 mb-3 border-b pb-2">Descrição</h2>
                                    <p className="leading-relaxed text-sm whitespace-pre-wrap text-gray-600">
                                ...
                                <a
                                    ...
                                    className="w-full py-4 text-white rounded-xl font-black flex items-center justify-center gap-3 hover:brightness-110 active:scale-[0.98] transition-all shadow-xl uppercase text-xs tracking-widest cursor-pointer"
                                    style={{
                                ...
                                >
                                    ...
                                    {product.available ? 'Solicitar Orçamento Pelo WhatsApp' : 'Solicitar Reserva / Consultar Prazo'}
                                </a>
                            </div>
                        </div>
                    </div>
