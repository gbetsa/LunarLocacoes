'use client';

import { useSettings } from '@/context/WhatsAppContext';

interface ProductContactButtonProps {
    productName: string;
    isAvailable: boolean;
}

export default function ProductContactButton({ productName, isAvailable }: ProductContactButtonProps) {
    const { getWhatsAppLink, refreshSettings } = useSettings();

    const handleAction = async () => {
        const { whatsapp: latestWhatsapp } = await refreshSettings();
        const message = isAvailable
            ? `Olá! Gostaria de solicitar um orçamento para o item: ${productName}`
            : `Olá! Tenho interesse no item ${productName}, porém vi no site que está indisponível no momento. Gostaria de solicitar uma reserva ou saber a previsão de disponibilidade.`;
        
        window.open(getWhatsAppLink(latestWhatsapp, message), '_blank');
    };

    return (
        <button
            onClick={handleAction}
            className="w-full py-3.5 sm:py-4 text-white rounded-xl font-black flex items-center justify-center gap-2 sm:gap-3 hover:brightness-110 active:scale-[0.98] transition-all shadow-xl uppercase text-[10px] sm:text-xs tracking-widest cursor-pointer"
            style={{
                background: isAvailable ? '#22c55e' : '#1e3a8a',
                boxShadow: isAvailable ? '0 10px 20px -5px rgba(34, 197, 94, 0.4)' : '0 10px 20px -5px rgba(30, 58, 138, 0.3)'
            }}
        >
            <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
                <path d="M12 0C5.373 0 0 5.373 0 12c0 2.124.558 4.118 1.534 5.845L.057 23.492a.5.5 0 00.626.606l5.775-1.515A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.882a9.877 9.877 0 01-5.032-1.374l-.36-.214-3.733.979.996-3.638-.235-.374A9.856 9.856 0 012.118 12C2.118 6.56 6.56 2.118 12 2.118S21.882 6.56 21.882 12 17.44 21.882 12 21.882z" />
            </svg>
            {isAvailable ? 'Solicitar Orçamento' : 'Solicitar Reserva'}
        </button>
    );
}
